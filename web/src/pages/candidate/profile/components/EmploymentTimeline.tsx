...
interface EmploymentTimelineProps {
  ...
  saveHistoryItem: (index: number) => void;
    ...
    >
      ...
      <div className="relative border-l-2 border-slate-150 ml-4 pl-6 space-y-8">
        {employmentHistory.map((item, index) => (
          <div key={`${item.company}-${item.designation}-${index}`} className="relative">
            {/* Timeline bullet dot */}
                ...
                <div className="bg-slate-50 p-4 rounded border border-slate-200 space-y-3 mt-2">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold text-slate-500 uppercase text-left w-full block">
                        Designation
                      </label>
                      <input
                        type="text"
